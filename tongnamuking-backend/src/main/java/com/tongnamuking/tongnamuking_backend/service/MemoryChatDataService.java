...
public class MemoryChatDataService {
     ...
     */
    public List<ChatStatsResponse> getChatStatsByChannelAndTimeRange(HttpServletRequest request, String channelName,
            double hours) {
        String clientId = clientIdentifierService.resolveClientId(request);
    ...
    public void addChatMessage(String clientId, String username, String channelName, String message) {
        ...
        Map<String, List<ChatData>> channelData = clientChatData.computeIfAbsent(clientId,
                k -> new ConcurrentHashMap<>());

        // 채널별 채팅 리스트 가져오기 또는 생성
        List<ChatData> chatList = channelData.computeIfAbsent(channelName,
    ...
    private void cleanupOldestChannelData(String clientId) {
        ...
        String oldestChannel = channelData.entrySet().stream()
                .min((e1, e2) -> {
                    LocalDateTime time1 = e1.getValue().isEmpty() ? LocalDateTime.now()
                            : e1.getValue().get(0).timestamp;
                    LocalDateTime time2 = e2.getValue().isEmpty() ? LocalDateTime.now()
                            : e2.getValue().get(0).timestamp;
    ...
    public List<ChatStatsResponse> getChatStatsByChannelAndTimeRange(String clientId, String channelName,
            double hours) {
        Map<String, List<ChatData>> channelData = clientChatData.get(clientId);
        if (channelData == null) {
            return new ArrayList<>();
